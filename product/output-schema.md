...
⚠️ **`alertHistory` 由 producer 每轮维护，不是初始化的快照。**
早先它只有 `init.js` 写过，运行期没有任何 producer 更新 —— 实测后果：
DOGE 的 kline 走到 2026-08-25 而记录停在 08-21，中间的触发一条都不在里面，
Tab 2 的历史标记因此系统性少报。现在：

```
PV1  日线 producer 每天整天替换
PV5  盘中 producer 每 15 分钟整天替换
```

**整天替换，不是增量追加**：两个 producer 每轮都重新评估当天全部的 bar，
手里就是当天的完整名单，替换是幂等的而追加不是（盘中每 15 分钟跑一次，
追加会把同一根 bar 记 96 遍）。同一天同一族只有一条 —— 图上一天一个位置。
**没触发也要走一遍**，否则上一轮误记的那条永远撕不掉。

⚠️ **`n` 与 `bars.length` 常态就不相等，这是声明，不是缺陷。**

```
n     = 当天真实触发次数
bars  = 其中最强的 8 根（按 |z| 取）
```

加密一天最多 96 根 bar，明细不可能全存。让两个数不等这件事写在契约里，
比留给读者自己发现好 —— 这个项目在「两个数对不上」上栽过不止一次。

⚠️ **`historicalTriggers` 不再是权威来源。** 它只有 init 写过，
而标记这一侧既读 `alertHistory` 又补当天的 findings —— 同一件事存两份、
其中一份被补过，自检因此长期报「与画出来的对不上」。
**界面上的计数从画出来的标记现数**，数字与标记构造上一致。
契约里那份保留作兜底（老数据、或这一版之前的产物），
与现数的差额本身有信息：它是这个 playbook 自初始化以来又响过的次数。
