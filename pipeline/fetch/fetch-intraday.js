...
(async()=>{
 ...
 const {sym, crypto, days} = env.args;
 const t1=Math.floor(new Date("2026-08-22T00:00:00Z").getTime()/1000);
 const D=days||135, CH=crypto?20:45;               // 加密一段 20 天，美股 45 天
 const rows=[]; let errs=0;
 for(let off=0; off<D; off+=CH){
   const b=t1-Math.min(D,off+CH)*86400, e=t1-off*86400;
   const u = crypto
