...
def main():
    ...
    with gzip.open(f"{BASE}/derived/m18_labels.json.gz", "wt", encoding="utf-8") as f:
        json.dump(list(labels.values()), f, ensure_ascii=False)
    with gzip.open(f"{BASE}/derived/m18_raw.json.gz", "wt", encoding="utf-8") as f:
        json.dump(raws, f, ensure_ascii=False)
    with open(f"{BASE}/derived/m18_stats.json", "w") as f:
