...
def main():
    raws = json.load(gzip.open(f"{DERIV}/m18_raw.json.gz", "rt", encoding="utf-8"))
    meta = json.load(gzip.open(f"{DERIV}/llm_meta.json.gz", "rt", encoding="utf-8"))
    passed = {o["id"]: o for o in json.load(gzip.open(f"{DERIV}/m18_labels.json.gz", "rt", encoding="utf-8"))}
    out, badparse = {}, 0
    for r in raws:
        try:
    ...
    with gzip.open(f"{DERIV}/m18_full.json.gz", "wt", encoding="utf-8") as f:
