...
EVSETS = {
    "物价 CPI": [rd for rd, _, _ in first_release("CPI")],
    "就业 NFP": [rd for rd, _, _ in first_release("TOTAL_NONFARM_PAYROLL")],
    "产出 GDP": [rd for rd, _, _ in first_release("GDP")],
    "有效联邦基金利率": [rd for rd, _, _ in first_release("FEDERAL_FUNDS")],
    "FOMC(手工日历)": FOMC,
}
OFFSETS = [0, -3, -6, 3]        # 月份偏移
...
if __name__ == "__main__":
    ...
    with Pool(9) as p:
        for o in p.imap_unordered(work, jobs):
            if o: res[o[0]] = o[1]
    json.dump(res, open(f"{OUT}/posmatch.json", "w"), ensure_ascii=False)
