...
它建出来的东西对不对。11 个案例、17 轮真跑、六层判官、73 条缺陷记录。
...
| [`traffic/`](traffic/) | GitHub 访问记录（Actions 每天抄一次，因为 GitHub 只留 14 天） |
...
```bash
python3 backtest/scripts/checks/check_consistency.py   # 规格 · 契约 · 文案 · producer 冒烟，共 24 项
...
判官逐条见 [`eval/judges.md`](eval/judges.md)（51 条断言，由脚本从判官代码生成）。
...
[MIT](LICENSE)。**这不是投资建议** —— 它是一个信号与告警的工程演示，不对任何数字的准确性作担保，也不应作为交易依据。
