...
CASES = [
    ...
    dict(
        id="BC69", group="copy", sev="med", owner="页面",
        caught=False, fixed="已修 · v1.5.4 已发布",
        title="新闻行只印时刻不印日期，跨 ET 日之后看起来像乱序",
        field=".nrow 的时刻", actual="`00:09` 排在 `13:26` 上面",
        expect="`Aug 24 00:09 ET` 排在 `Aug 23 13:26 ET` 上面",
        onpage="读者只能认为排序坏了 —— 而它是对的",
        cause="`2026-08-24T04:09Z` 渲染成 00:09，`2026-08-23T17:26Z` 渲染成 13:26。"
              "文件本身是按 ISO 倒序的，两行跨了一个 ET 日，而行上只印时刻。",
        why="⚠️ **这个仓库已经为这件事定过规则，写在 `clock` 上方：**"
            "「当天的只印时刻、跨日的带上日期」那个条件式做法被否决过一次，"
            "结论是**日期一律印** —— 「没有日期」这个信号只有已经知道规则的人才读得出来。"
            "告警卡遵守了，新闻 feed 没有。\n\n"
            "⚠️ 断言也栽在同一处：它抓 `\\d{1,2}:\\d{2}` 然后**按字符串比大小**，"
            "于是和读者得出同一个错误结论。修的时候两件事要分开："
            "顺序按行自己的 `data-at`（ISO）比，日期可见性单独查。",
        fix=["新闻行加 `etDate(n.at)`，日期一律印",
             "行上带 `data-at`（ISO）；断言按它比顺序，不按渲染字符串",
             "⚠️ 另加一条断言：每行的可见时刻必须带日期 —— "
             "否则「顺序对了但读者看不出来」这一半没人查"],
        assertion="`auditNewsChain` 两条：按 data-at 的顺序 · 每行可见日期。两条都做过破坏性测试。",
        verified="线上数据（12 条新闻横跨 08-23 与 08-24）：两条断言各造反例都真的失败过一次。",
    ),
    dict(
        id="BC70", group="selfcheck", sev="med", owner="判官（我）",
        caught=False, fixed="已修 · v1.5.4 已发布",
        title="两条自检：一条不知道契约的豁免，一条靠巧合通过",
        field="auditContractShape · auditPageTimes", actual="前者误报 4 条，后者一直是真空",
        expect="前者认豁免并反向查，后者登记派生时刻",
        onpage="不上页面 —— 但一个误报的自检会让人开始忽略自检",
        cause="① `auditContractShape` 要求每条 finding 都有全部 CONTEXT_KEYS，"
              "而契约明写「US1–3 无 attribution」，`userlines.js` 也刻意不写。\n"
              "② `auditPageTimes` 的白名单只收原始字段的时刻，"
              "而告警窗口边界是**派生**的（当前 UTC 日零点）。",
        why="⚠️ **第二条是靠巧合通过的。** mock 里 `META.now` 恰好是 20:00 ET，"
            "而窗口边界印出来也是 20:00 —— 于是它一直在白名单里。"
            "换成真实数据（21:20）巧合没了，它才第一次响。"
            "**在真实数据上跑一次，是发现这类断言的唯一办法**，"
            "定值数据会让它们看起来一直健康。\n\n"
            "⚠️ 第一条的修法不能只是「跳过」：豁免的键**出现了**同样是错，"
            "否则这条豁免就是一张免检牌，哪天 producer 真写了个假的 attribution 上去也不会响。",
        fix=["auditContractShape 明列豁免，并加反向检查（豁免的键出现了也报）",
             "auditPageTimes：窗口边界由 `alertWindowStartISO()` 统一产出，"
             "渲染和白名单读同一个函数，两处不可能再分叉"],
        assertion="两条都做过破坏性测试：塞一个 attribution 进 US finding、"
                  "往页面塞一个无来源的 03:47，各自都真的失败过一次。",
        verified="线上数据上 45 条自检只剩 auditFindingSource ——"
                 "那是日线 producer 尚未按新调度开火的真实症状，不是缺陷。",
    ),
    dict(
        id="BC71", group="copy", sev="med", owner="脚本 + 检查器",
        caught=False, fixed="已修 · 已部署",
        title="带载荷的 gap 只发不撕，条件消失之后它还在说当时为真的话",
        field="findings.gaps · meta.gaps", actual="15 只全部 08-24，而 gap 仍写「跨 08-21 与 08-23」",
        expect="拥有者在条件消失时撕掉自己那一条",
        onpage="缺口说明照常渲染，读者据此以为账本还跨着两个交易日",
        cause="`producer.js` 的 `gaps` 从上一轮继承、只 push 不 remove，"
              "落 findings 时又与 `prevFj.gaps` 取并集 —— 两处都只并不清。",
        why="⚠️ **是每日核查的定时任务发现的，不是人也不是判官。** "
            "日线 cron 第一次按新调度开火之后两类同日，跨日条件消失，而 gap 还在。\n\n"
            "⚠️ 文件顶上写着「谁发现谁追加，没有人整体清空」——"
            "那是为了防止一个 producer 抹掉别人记的缺口，方向没错，"
            "**但它漏了一种情况：拥有者撤回自己那一条。**\n\n"
            "⚠️ 修的时候发现隔壁 `fetch_errors:` 同一个毛病。"
            "只修撞见的那一条，另一条会在下一次取数成功时以同样的方式变成谎话。\n\n"
            "⚠️ **检查器本来该拦住它，但它的判据是名字。** "
            "`_owed` 认 `not_yet` / `not_run` / `pending` 三个词，"
            "而 `holdings_span_multiple_sessions` 一个都不占 —— 这个检查**从没查过它**。"
            "补第二条判据：带载荷的 gap（`xxx:<这一轮算出来的值>`）断言的是一次具体观察，"
            "构造上不可能是永久边界。名字是约定，载荷是结构。\n\n"
            "⚠️ 而且检查器认「撕」只认 `.delete(\"字面量\")`，看不见前缀过滤 ——"
            "**我的修法它也看不见**，会把已修的报成没修。两种写法都认之后，"
            "「有人撕」的计数从 1 变成 4。",
        fix=["producer.js 落盘前先剔除自己拥有的两个前缀，再决定要不要重发；"
             "findings 与上一轮取并集处同样先剔除，否则刚删的会被原样并回来",
             "检查器：判据加「带载荷」；撕法加「前缀过滤」",
             "⚠️ 判据放宽后又暴露 8 条同类欠条。**不缩判据让它闭嘴** ——"
             "列成明账 `_OWED_DEBT`，这张表只许变短，新增一条就红",
             "`unvalidated_asset_class` 归入 `_NOT_OWED`：它只有 init 发，"
             "运行期没人重算账本的资产类别构成，与 `m23_not_run` 同类"],
        assertion="`check_consistency` 的欠条检查，两条判据两种撕法。已做破坏性测试："
                  "塞一条无人撕的带载荷 gap，退出码 1，且被点名。",
    ...
    dict(
        id="BC73", group="copy", sev="med", owner="脚本 + 判官（我）",
        caught=False, fixed="已修 · 已部署",
        title="端点的内部码直接上了手机：「NVDA 2026-08-26 amc」",
        field="EV4 推送正文", actual="`amc`（端点内部码）", expect="`after close`",
        onpage="页面上是对的 —— 只有推送这一路没翻",
        cause="`producer-context.js` 的推送把 `${e.time}` 原样拼进正文。"
              "端点给的是 `bmo` / `amc`，那是它的内部码。",
        why="⚠️ **同一个事实在页面上有名字，在推送里没有。** 页面早有 "
            "`earnBmo`「before open」/ `earnAmc`「after close」，"
            "而推送恰恰是唯一会打断用户的那个出口。\n\n"
            "⚠️ CLAUDE.md 的写作约定第 2 条写着「不裸用字母代号」——"
            "这条约定此前只在文档与页面上被执行过，**推送从来不在它的射程里**。\n\n"
            "⚠️ 其余两路早就做对了（`US1` → stop line、`US2` → take-profit line），"
            "所以这不是没想到，是**漏了一处而没有任何东西会发现**。",
        fix=["`{bmo:'before open', amc:'after close'}` 映射；认不出的值原样带出去，不吞掉",
             "⚠️ **冒烟测试截获推送**：`ts().append` 原来是空函数，"
             "推送正文从来没被任何检查看过",
             "判据是「这个词是给机器看的还是给人看的」：bmo/amc · us_equity · "
             "L1–L4 · chain/model · 裸信号 ID",
             "⚠️ 光加断言不够 —— 桩里 HTTP 一律返回空，EV4 永远不推，"
             "那条断言会是个不会失败的断言。给财报日历喂一条**明天**的记录，"
             "`time` 故意用原始码 `amc`：要测的正是它有没有被翻成人话",
             "⚠️ 第一版日期用 `Date.now()+1d`，而种子的 asOf 钉在 2026-08-23，"
             "差好几天 → EV4 的「≤1 个交易日」不成立 → 断言静默没求值。"
             "改成从种子自己的 asOf 推，日期只有一处来源"],
        assertion="冒烟测试「推送正文里没有内部码」。⚠️ 未推送时明写「本判据未求值」——"
                  "0 条要看得见，否则「查过了没问题」和「压根没推」长得一样。"
                  "破坏性测试：撤掉映射 → 退出码 1，且原文被打印出来。",
        verified="四个 producer 的推送逐一扫过：日线与盘中早已把 US1–3 翻成人话，"
                 "EV4 是唯一漏的一处。已部署 producer-context.js，逐字节核对一致。",
    ),
]
